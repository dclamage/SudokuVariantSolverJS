...
export class SkyscraperConstraint extends Constraint {
    ...
    bruteForceStep(board: Board): ConstraintResult {
        ...
        const newMasks = Array.from({ length: size }, () => 0);
        ...
        for (let i = size - 1; i >= 0; i--) {
            ...
            for (const prevState of possiblePrevStates) {
                for (const curState of curStates) {
                    newMasks[i] |= SkyscraperConstraint.stateStateToMask[size][prevState * numStates + curState];
                }
            }
        ...
        return board.applyCellMasks(this.cells, newMasks);
    }
